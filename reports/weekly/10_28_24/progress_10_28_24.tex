...
\begin{document}
    ...
    However, this results in the error \texttt{The size of tensor a (4096) must match the size of tensor b (3968) at non-singleton dimension 2}.
    The fundamental issue is that I don't have a proper understanding of how a ``head'' subdivides the 3 layers and combines them.
    I've located and begun studying the source code for LlamaSdpaAttention in order to figure out how to properly prune the heads.
    I'll also be studying the ``MultiheadAttentionPruner'' in LLM-Pruner~\cite{LLM-Pruner} to better understand head-pruning.
    My current attempt at pruning can be found in my ``llm-test-runner'' repo~\cite{llm-test-runner}.
